...
import { DEFAULT_CONFIG, Frame, RESOLUTION_LIST } from '@/config';
...
function SettingButton(props: Props) {
  ...
  const [resolution, setResolution] = useState(DEFAULT_CONFIG.resolutionsText);
  const [frameRate, setFrameRate] = useState(DEFAULT_CONFIG.frameRate);

  const handleOk = () => {
    setVisible(false);

    const resolutions = RESOLUTION_LIST.find((r) => r.text === resolution);
